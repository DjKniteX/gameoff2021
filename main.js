(function (w) {
    ...
    items.forEach(function (i, idx) {
      ...
      attach(
        ...
        el(
          "li",
          {
            onclick: selectedInventory.bind(null, i, idx, items),
  ...
  function handlePointing(ev) {
    ...
    const qs = Math.ceil(
      (Math.floor((Math.atan2(y, x) + Math.PI) / (Math.PI / 4.0)) % 7) / 2
    );
  ...
  document.querySelectorAll(".game-title-subText").forEach(function (t) {
    t.textContent = subGametitle;
  });
